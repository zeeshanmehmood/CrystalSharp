...
{
    [Trait(TestSettings.Category, TestType.MySqlEventStoreDbIntegration)]
    public class MySqlEventStoreDbPersistenceTests(MySqlEventStoreTestFixture fixture) : IClassFixture<MySqlEventStoreTestFixture>
    {
        private readonly MySqlEventStoreTestFixture _testFixture = fixture;
        ...
        public async Task Raise_stream_not_found_exception()
        {
            ...
            // Act
            Func<Task<Product>> result = async () => await sut.Get<Product>(Guid.NewGuid(), TestContext.Current.CancellationToken).ConfigureAwait(false);
